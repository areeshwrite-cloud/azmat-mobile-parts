...
export default function Footer({ config }) {
  const phone = config?.contact_phone || '0319-7900202';
  const whatsapp = config?.whatsapp || '0319-7900202';
  const whatsappDigits = whatsapp.replace(/\D/g, '');
  const email = config?.email || 'noreply@azmatmobileparts.com';
  const address = config?.address || 'NEW MADYAN ROAD QADRIA MARKET SHOP NO 21 22 AZMAT MOBILE PARTS MINGORA SWAT.';
    ...
    <footer className="bg-[#071426] text-slate-300">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              ...
              <li className="flex items-center gap-2">
                ...
                <a
                  href={`https://wa.me/${whatsappDigits}?text=${encodeURIComponent('Salam! I have a question regarding Azmat Mobile Parts products.')}`}
                  target="_blank"
                  rel="noreferrer"
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-3 px-4 py-5 text-xs sm:flex-row sm:justify-between">
          ...
          <p className="text-slate-400">{config?.footer_text || '© Azmat Mobile Parts. All rights reserved.'}</p>
        </div>
      </div>
    </footer>
  );
